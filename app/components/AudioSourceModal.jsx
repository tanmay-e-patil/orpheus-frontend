...
export const AudioSourceModal = ({ song, closeAudioSourceModal, addVideoDetailsToSongObject }) => {
    ...
    useEffect(() => {
        ...
        const videoItems = youtubeSearch.data ? youtubeSearch.data.items.filter(item => item.id.videoId) : []
    ...
    if (youtubeSearch.error || ytVideosContentDetails.error) {
        return <Text>Error: {youtubeSearch.error.message || ytVideosContentDetails.error}</Text>;
    }

    //TODO: Call yt v3 api to get content details
    const getSongDuration = (videoId) => {
        const video = ytVideosContentDetails.data?.items.find(i => i.id === videoId)
    ...
    return (
        <View className="mx-2 mt-4">
            <FlatList data={youtubeSearch.data?.items.filter(item => item.id.videoId) ?? YOUTUBE_SEARCH_DATA.items} keyExtractor={(item) => item.id?.videoId}
                renderItem={({ item }) => (
                    <View>

                        <TouchableOpacity className="py-2" onPress={async () => {
                            addVideoDetailsToSongObject(item.id?.videoId, getSongDuration(item.id?.videoId))
                    ...
                    <View className="my-6 pr-4 space-y-6">
                        <View className="justify-center items-center flex-row w-full">
                            ...
                            <View className="flex-row justify-center items-center">
                                <TouchableOpacity className="    border-2 bg-slate-50 rounded-full" onPress={closeAudioSourceModal}>
                                    <Text className="text-black text-lg  font-psemibold w-full p-2 px-4">X</Text>
                                </TouchableOpacity>
                                <Text className="text-white text-3xl font-pbold ml-2">Select audio source</Text>
                            </View>
                        </View>
                    </View>)}
            >
