...
const SongCard = ({
  ...
  song: SongProps;
  handleTrackSelect: (song: SongProps) => void;
}) => {
    ...
    <TouchableOpacity onPress={() => handleTrackSelect(song)}>
      <View className="flex-row space-x-4 items-center pr-4">
        ...
        <View className="flex-col justify-center">
          <Text
            numberOfLines={1}
            className="w-48 font-psemibold text-lg text-white px-4"
          >
            {song.title}
          </Text>
          <Text
            numberOfLines={1}
            className="w-48 font-psemibold text-sm text-gray-100 px-4"
          >
            {song.artist}
          </Text>
          <Text
            numberOfLines={1}
            className="w-48 font-pregular text-sm text-gray-100 px-4"
          >
            {song.duration}
          </Text>
        </View>
