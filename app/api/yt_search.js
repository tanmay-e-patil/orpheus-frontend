...
export const searchYoutube = async (query) => {
  ...
  const response = await fetch(`${YOUTUBE_SEARCH_URL}?part=snippet&maxResults=10&q=${encodeURIComponent(query)}&key=${API_KEY}`)
...
export const getContentDetails = async (videoIds) => {
  ...
  const response = await fetch(`${YOUTUBE_VIDEO_DETAILS_URL}?part=contentDetails&id=${videoIds}&key=${API_KEY}`)
