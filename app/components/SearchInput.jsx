...
const SearchInput = ({ title, value, placeholder, handleChangeText, onSearch, otherStyles, ...props }) => {
    ...
    return (
            <View className="w-full h-16 px-4 bg-black-100 rounded-2xl focus:border-secondary items-center border-black-200 flex-row space-x-4">
                <TextInput className="mt-0.5 font-pregular text-white text-base flex-1" value={value} placeholder={placeholder}
                           placeholderTextColor="#7b7b8b" onChangeText={handleChangeText} secureTextEntry={title === 'Password' && !showPassord} onSubmitEditing={onSearch}></TextInput>
                <TouchableOpacity>
